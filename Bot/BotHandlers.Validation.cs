...
public sealed partial class BotHandlers
{
    internal static bool TryNormalizeLanguage(string input, out string lang)
    {
        ...
        if (lang is "fi" or "en") return true;
        lang = string.Empty;
        return false;
    }
    ...
    internal static bool LooksLikeRealName(string input)
    {
        ...
        foreach (var p in parts)
        {
            ...
            foreach (var ch in p)
            {
                if (!(char.IsLetter(ch) || ch is '-' or '\'')) return false;
            }
        }
        return true;
    }
    ...
    internal static string NormalizeName(string input)
    {
        var parts = input
            ...
            .Select(p => char.ToUpperInvariant(p[0]) + (p.Length > 1 ? p[1..].ToLowerInvariant() : string.Empty));
    ...
    internal static bool TryNormalizeHandle(string input, out string handle)
    {
        ...
        if (handle.Length < 5 || handle.Length > 32) return false;
        if (!handle.All(ch => char.IsLetterOrDigit(ch) || ch == '_')) return false;
        return true;
    }
