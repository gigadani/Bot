...
public static class CsvExporter
{
    public static async Task<int> ExportAsync(string? outputPath = null, string? inputPath = null, CancellationToken ct = default)
    {
        ...
        await using FileStream inStream = File.Open(inputPath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
        ...
        await using FileStream outStream = File.Open(outputPath, FileMode.Create, FileAccess.Write, FileShare.Read);
        ...
        foreach (GuestRecord? rec in latest.Values.Where(r => !string.Equals(r.Status, "Deleted", StringComparison.OrdinalIgnoreCase)))
        {
